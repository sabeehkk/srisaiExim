...
// Assuming these are correctly imported from your project's constants file
import { PRIMARY_COLOR, ACCENT_COLOR, SERVICE_CARDS } from "@/lib/constants"; 
import { servicesData } from "@/lib/constants"; 
import { X, CheckCircle } from "lucide-react"; 

// Animation duration constant (Must match the CSS transition duration in Tailwind)
const DURATION = 300; 
const TRANSITION_CLASSES = `transition-all duration-300 ease-out`;

// --- 1. ServiceModal Component (Handles visual animation based on props) ---
const ServiceModal = ({ service, onClose, isClosing }) => {
    
    const [isVisible, setIsVisible] = useState(false);
    // State to hold the last valid service data during the exit animation period
    const [modalData, setModalData] = useState(null); 

    // Effect to control animation visibility and store data
    useEffect(() => {
        let timer;

        if (service && !isClosing) {
            // OPENING: Store data, and start the entrance animation after a slight delay
            setModalData(service);
            timer = setTimeout(() => setIsVisible(true), 10);
            return () => clearTimeout(timer);
        } else if (isClosing) {
            // CLOSING: Trigger the exit animation immediately
            setIsVisible(false);
        } else if (!service && !isClosing && modalData) {
            // FINAL CLEANUP: When the parent unmounts (after DURATION), clear leftover data
            setModalData(null);
        }
    }, [service, isClosing]); 

    // Render only if we have data (either open or animating out)
    if (!modalData) return null;


    // Dynamic classes for the modal content (scale and opacity)
    const modalContentClasses = [
        "bg-white rounded-xl border border-gray-200 shadow-3xl shadow-cyan-500/10 w-full max-w-5xl mx-4 transform overflow-hidden max-h-[95vh]",
        TRANSITION_CLASSES, 
        // Use isVisible (controlled by the effect) for fade and scale
        isVisible ? "opacity-100 scale-100" : "opacity-0 scale-95", 
    ].join(' ');

    // Dynamic classes for the backdrop (opacity)
    const backdropClasses = [
        "fixed inset-0 z-[100] flex items-center justify-center bg-gray-900/80 backdrop-blur-sm",
        TRANSITION_CLASSES, 
    ...
    return (
        <div 
            ...
            onClick={onClose} // Calls parent's `closeModal` which handles the delay
        >
            <div 
            ...
            >
                ...
                <div className="p-8 md:p-10 border-b border-gray-100 flex justify-between items-start">
                    <h2 className="text-4xl lg:text-5xl font-bold text-gray-900 leading-tight">
                        <span className="text-cyan-600">Future-Proof</span> {modalData.title}
                    </h2>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 overflow-y-auto max-h-[70vh]">
...
// --- 2. Main Services Component (Controls the Delay and Scroll) ---
export default function Services() {
    const [selectedService, setSelectedService] = useState(null);
    const [isClosing, setIsClosing] = useState(false); // New state to manage the exit animation trigger

    const openModal = (service) => {
        setIsClosing(false); 
        setSelectedService(service);
        // 1. Lock body scroll immediately upon opening
        document.body.style.overflow = 'hidden'; 
    };

    const closeModal = () => {
        // 1. Trigger the visual exit animation in the child component
        setIsClosing(true); 
        
        // 2. Unlock scroll immediately so the background is scrollable during exit animation
        document.body.style.overflow = 'auto'; 

        // 3. Wait for the animation duration (300ms), then fully unmount the modal
        setTimeout(() => {
            setSelectedService(null);
            setIsClosing(false);
        }, DURATION); 
    ...
    return (
        <section className="py-28 bg-white"> 
            ...
            {/* The Modal Component - Pass isClosing state */}
            {/* The modal is rendered if data is present, or if it's currently closing */}
            <ServiceModal service={selectedService} onClose={closeModal} isClosing={isClosing} />
        </section>
    );
}